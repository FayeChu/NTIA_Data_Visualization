...
// let businessSvg = d3.select(".business").append("svg").attr("width", 425).attr("height", 120),
//     customerSvg = d3.select(".customer").append("svg").attr("width", 425).attr("height", 120),
//     mobileSvg = d3.select(".mobile").append("svg").attr("width", 425).attr("height", 120);

// let businessChart = businessSvg
//     .append("g")
//     .attr("width", 425-margin.left-margin.right)
//     .attr("height", 120-margin.top-margin.bottom),

//     customerChart = customerSvg
//     .append("g")
//     .attr("width", 425-margin.left-margin.right)
//     .attr("height", 120-margin.top-margin.bottom),

//     mobileChart = mobileSvg
//     .append("g")
//     .attr("width", 425-margin.left-margin.right)
//     .attr("height", 120-margin.top-margin.bottom)
...
function bubbleChart() {
    ...
    var typeCenters = {
      business: { x: width / 3, y: height / 1.5 },
      consumer: { x: width / 2, y: height / 1.5 },
      mobile: { x: 2 * width / 3, y: height / 1.5 }
    };
     ...
     */
    function createNodes(rawData, state, metric) {
      // Use the max total_amount in the data as the max in the scale's domain
      // note we have to ensure the total_amount is a number.
      console.log(rawData)
      rawData = rawData.filter((d) => {return +d.statecode == 36})
      var maxAmount = d3.max(rawData, function (d) { return +d.providername; });
      if(metric == "providername") {
        var maxAmount = d3.max(rawData, function (d) { return +d.providername; });
      } else {
        var maxAmount = d3.max(rawData, function (d) { return +d.providername; });
      }
      ...
      var myNodes = rawData.map(function (d) {
        return {
          id: d.type+'-'+d.tech,
          radius: radiusScale(+d.providername),
    ...
    var chart = function chart(selector, rawData) {
      ...
      bubbleData = d3.selectAll('.bubble')._groups[0];
    };
    ...
    function showTypeTitles() {
      ...
      // var typesDate = ["business", "consumer", "mobile"]
      var types = svg.selectAll('.type')
        .data(typesData);
  
      types.enter().append('text')
        .attr('class', 'type')
        .attr('x', function (d) { return typeTitleX[d]+30; })
        .attr('y', 40)
   ...
   */
  function display(error, data, state) {
    if (error) {
      console.log(error);
    }
  ...
  // Load the data.
  // d3.csv('./data/whole_nation_state.csv', display);
